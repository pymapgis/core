#!/usr/bin/env python3
"""
Create QGIS Project for Tennessee Counties

This script creates a complete QGIS project programmatically using PyQGIS.
It demonstrates how to:
1. Load Tennessee counties data
2. Style the layer with regional colors and labels
...
TENNESSEE_GPKG = DATA_DIR / "tennessee_counties.gpkg"
PROJECT_PATH = DATA_DIR / "tennessee_counties_project.qgz"
...
def check_prerequisites():
    ...
    if not TENNESSEE_GPKG.exists():
        print(f"❌ Tennessee counties data not found: {TENNESSEE_GPKG}")
        print("   Run tennessee_counties_example.py first to download the data")
        return False
...
def load_tennessee_counties():
    """Load Tennessee counties layer."""
    print("📂 Loading Tennessee counties data...")
    
    # Create vector layer
    layer = QgsVectorLayer(str(TENNESSEE_GPKG), "Tennessee Counties", "ogr")
    
    if not layer.isValid():
        print(f"❌ Failed to load layer from {TENNESSEE_GPKG}")
        return None
    
    print(f"✅ Loaded {layer.featureCount()} counties")
    return layer

def add_region_field(layer):
    """Add a region field to categorize counties by Tennessee regions."""
    print("🏔️  Adding regional classification...")
    
    # Start editing
    layer.startEditing()
    
    # Add region field if it doesn't exist
    fields = layer.fields()
    if not fields.indexFromName('region') >= 0:
        from qgis.core import QgsField
        layer.dataProvider().addAttributes([QgsField('region', QVariant.String)])
        layer.updateFields()
    
    # Update region field based on longitude
    for feature in layer.getFeatures():
        centroid = feature.geometry().centroid().asPoint()
        
        if centroid.x() > -85.5:
            region = 'East Tennessee'
        elif centroid.x() >= -87.5:
...
def style_counties_layer(layer):
    """Apply regional styling to the counties layer."""
    print("🎨 Styling counties layer with regional colors...")
    
    # Add region field
    add_region_field(layer)
    
    # Define colors for each region
    region_colors = {
        'East Tennessee': QColor(240, 128, 128),    # Light coral
        'Middle Tennessee': QColor(144, 238, 144),  # Light green
        'West Tennessee': QColor(135, 206, 250)     # Light sky blue
    }
    
    # Create categories for each region
    categories = []
    for region, color in region_colors.items():
        ...
        category = QgsRendererCategory(region, symbol, region)
    ...
    print("✅ Regional styling applied")
...
def create_layout(layer):
    ...
    try:
        ...
        layout.setName("Tennessee Counties Map")
...
def save_project():
    ...
    # Set project title
    project.setTitle("Tennessee Counties Analysis")
...
def main():
    ...
    print("🏛️  Creating Tennessee Counties QGIS Project")
    print("=" * 50)
    ...
    try:
        ...
        if save_project():
            ...
            print(f"• 95 Tennessee counties with regional classification")
            print(f"• Color-coded by region (East/Middle/West Tennessee)")
            print(f"• County name labels")
            print(f"• Print layout ready for export")
            print(f"")
...
def print_usage_info():
    ...
    print("")
    print("This script creates a QGIS project with Tennessee counties data.")
    print("Make sure to run tennessee_counties_example.py first to download the data.")
    print("")
    print("Requirements:")
