...
class _Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="PYMAPGIS_", extra="ignore", env_file_encoding='utf-8')
    ...
    def settings_customise_sources(
        ...
        for toml_file_path in toml_files_to_check:
            if toml_file_path.exists():
                try:
                    loaded_toml_sources.append(TomlConfigSettingsSource(settings_cls, toml_file_path))
                except Exception as e:
                    # Handle potential errors during TOML file loading gracefully
                    print(f"Warning: Could not load settings from {toml_file_path}: {e}")
