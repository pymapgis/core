...
class PerformanceMonitor:
    ...
    def finish(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "mb_per_second": (self.bytes_processed / 1024 / 1024) / duration if duration > 0 else 0,
            "memory_start_mb": self.memory_start,
            "memory_peak_mb": self.memory_peak,
            "memory_increase_mb": self.memory_peak - self.memory_start if self.memory_start else 0,
        }
        
        logger.info(f"Completed {self.name}: {self.items_processed} items in {duration:.2f}s "
...
class ChunkedFileReader:
    ...
    async def _read_csv_chunks(self, filepath: Path, **kwargs) -> AsyncIterator[pd.DataFrame]:
        """Read CSV in chunks."""
    ...
    async def _read_vector_chunks(self, filepath: Path, **kwargs) -> AsyncIterator[gpd.GeoDataFrame]:
        ...
        try:
            async for chunk in reader.read_file_async(filepath):
                ...
                result = await loop.run_in_executor(
                    self.executor,
                    lambda: operation(chunk, **operation_kwargs)
                )
                ...
                monitor.update(len(chunk) if hasattr(chunk, '__len__') else 1, chunk_size_bytes)
    ...
    async def _write_chunk_to_file(self, chunk: Any, output_path: Path, is_first: bool):
        ...
        if suffix == '.csv':
            ...
            if is_first:
                ...
                existing = await loop.run_in_executor(None, gpd.read_file, str(output_path))
