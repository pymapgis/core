...
class AdvancedCache:
    ...
    def _evict_memory_lru(self) -> None:
        ...
        while self._get_memory_usage() > self.memory_limit_mb and self.memory_cache:
            # Find LRU item
            lru_key = min(self.memory_access_time.keys(), 
...
class QueryOptimizer:
    ...
    def optimize_spatial_join(self, left_gdf, right_gdf, how='inner', predicate='intersects'):
    ...
    def get_profile_summary(self, name: str = None) -> Dict[str, Any]:
        ...
        if name and name in self.profiles:
            ...
            return {
                ...
                'duration': {
                    'mean': np.mean(durations) if NUMPY_AVAILABLE else sum(durations) / len(durations),
                    'min': min(durations),
                    'max': max(durations),
                    'total': sum(durations)
                },
                'memory': {
                    'mean_delta': np.mean(memory_deltas) if NUMPY_AVAILABLE else sum(memory_deltas) / len(memory_deltas),
    ...
    def _optimization_loop(self) -> None:
        ...
        while self.running:
            try:
                ...
                if cleanup_result:
                    logger.debug(f"Auto cleanup freed {cleanup_result['memory_freed_mb']:.1f}MB")
