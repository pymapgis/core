...
class S3Storage(CloudStorageBase):
    ...
    def __init__(self, bucket: str, region: str = None, **kwargs):
        ...
        if not AWS_AVAILABLE:
            raise CloudStorageError("AWS SDK (boto3) not available. Install with: pip install boto3")
...
class GCSStorage(CloudStorageBase):
    ...
    def __init__(self, bucket: str, project: str = None, **kwargs):
        ...
        if not GCS_AVAILABLE:
            raise CloudStorageError("Google Cloud SDK not available. Install with: pip install google-cloud-storage")
...
class AzureStorage(CloudStorageBase):
    ...
    def __init__(self, account_name: str, container: str, account_key: str = None, **kwargs):
        super().__init__(**kwargs)
        if not AZURE_AVAILABLE:
            raise CloudStorageError("Azure SDK not available. Install with: pip install azure-storage-blob")
    ...
    def list_files(self, prefix: str = "", max_files: int = 1000) -> List[Dict[str, Any]]:
        ...
        blobs = container_client.list_blobs(name_starts_with=prefix, max_results=max_files)
...
class CloudStorageManager:
    ...
    def register_s3(self, name: str, bucket: str, region: str = None, **kwargs) -> S3Storage:
        """Register S3 storage provider."""
        provider = S3Storage(bucket=bucket, region=region, **kwargs)
        self._providers[name] = provider
        return provider
        
    def register_gcs(self, name: str, bucket: str, project: str = None, **kwargs) -> GCSStorage:
        """Register GCS storage provider."""
        provider = GCSStorage(bucket=bucket, project=project, **kwargs)
        self._providers[name] = provider
        return provider
        
    def register_azure(self, name: str, account_name: str, container: str, 
                      account_key: str = None, **kwargs) -> AzureStorage:
        """Register Azure storage provider."""
        provider = AzureStorage(account_name=account_name, container=container, 
                              account_key=account_key, **kwargs)
        self._providers[name] = provider
        return provider
...
def register_s3_provider(name: str, bucket: str, region: str = None, **kwargs) -> S3Storage:
    """Register S3 provider globally."""
    return _cloud_manager.register_s3(name, bucket, region, **kwargs)


def register_gcs_provider(name: str, bucket: str, project: str = None, **kwargs) -> GCSStorage:
    """Register GCS provider globally."""
    return _cloud_manager.register_gcs(name, bucket, project, **kwargs)


def register_azure_provider(name: str, account_name: str, container: str,
                           account_key: str = None, **kwargs) -> AzureStorage:
    """Register Azure provider globally."""
    return _cloud_manager.register_azure(name, account_name, container, account_key, **kwargs)
...
class CloudDataReader:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        self.cache_dir = Path(cache_dir) if cache_dir else Path(tempfile.gettempdir()) / "pymapgis_cloud_cache"
    ...
    def read_cloud_file(self, cloud_url: str, provider_name: str = None, **kwargs):
        ...
        try:
            ...
            if suffix == '.csv':
                ...
                raise CloudStorageError(f"Cannot read file type {suffix} without PyMapGIS IO module")
...
def cloud_write(data, cloud_url: str, provider_name: str = None, **kwargs):
    ...
    if scheme == 's3':
        bucket = parsed.netloc
        path = parsed.path.lstrip('/')
        provider = _cloud_manager.get_provider(provider_name) if provider_name else S3Storage(bucket=bucket)

    elif scheme == 'gs':
        bucket = parsed.netloc
        path = parsed.path.lstrip('/')
        provider = _cloud_manager.get_provider(provider_name) if provider_name else GCSStorage(bucket=bucket)
        ...
        container = path_parts[0]
        path = path_parts[1] if len(path_parts) > 1 else ''
        provider = _cloud_manager.get_provider(provider_name) if provider_name else AzureStorage(account_name=account_name, container=container)

    else:
        raise CloudStorageError(f"Unsupported cloud URL scheme: {scheme}")

    # Write to temporary file first
    with tempfile.NamedTemporaryFile(suffix=Path(path).suffix, delete=False) as tmp_file:
        ...
        try:
            ...
            if hasattr(data, 'to_file'):
                ...
                raise CloudStorageError(f"Unsupported data type for cloud writing: {type(data)}")
...
def list_cloud_files(cloud_url: str, provider_name: str = None, max_files: int = 1000) -> List[Dict[str, Any]]:
    """
    List files in cloud storage.
