"""
FastAPI Web Application for {{DEMO_TITLE}}

Serves:
- GET / - Interactive web viewer
- GET /health - Health check endpoint  
- GET /public/tiles/{z}/{x}/{y}.mvt - Vector tiles (public access)
- GET /internal/latest - Full GeoJSON data (JWT protected, optional)
- GET /static/{filename} - Static files (PNG overview, etc.)
"""
...
def load_latest_data():
    ...
    try:
        if Path("output.geojson").exists():
            with open("output.geojson", "r") as f:
                latest_data = json.load(f)
            last_updated = Path("output.geojson").stat().st_mtime
            logger.info("Loaded latest processed data")
        else:
            logger.warning("No output.geojson file found")
...
async def get_latest_data(user=Depends(jwt_verify)):
    """
    Get latest processed data (JWT protected).
    """
...
async def health_check():
    ...
    data_status = "ok" if Path("output.geojson").exists() else "no_data"
    tiles_status = "ok" if Path("tiles/{{LAYER_NAME}}").exists() else "no_tiles"
...
async def get_static_file(filename: str):
    """
    Serve static files (PNG overview, etc.).
    """
    ...
    # Check in root directory for generated files
