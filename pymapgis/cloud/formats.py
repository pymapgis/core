...
class CloudOptimizedWriter:
    ...
    def write_cog(self, data: xr.DataArray, output_path: str, **kwargs) -> None:
        ...
        if kwargs.get('add_overviews', True):
            cog_options['OVERVIEW_RESAMPLING'] = kwargs.get('overview_resampling', 'average')
        
        # Write with COG profile
        data.rio.to_raster(output_path, **cog_options)
        logger.info(f"Wrote Cloud Optimized GeoTIFF: {output_path}")
    
    def write_geoparquet(self, data: gpd.GeoDataFrame, output_path: str, **kwargs) -> None:
        """
    ...
    def write_flatgeobuf(self, data: gpd.GeoDataFrame, output_path: str, **kwargs) -> None:
        ...
        try:
            ...
            logger.warning(f"FlatGeobuf writing failed, falling back to GeoParquet: {e}")
...
class CloudOptimizedReader:
    ...
    def read_zarr_slice(self, zarr_path: str, time_slice: Optional[slice] = None,
