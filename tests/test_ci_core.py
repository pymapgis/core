...
def test_example_structure():
    ...
    if tennessee_dir.exists():
        ...
        assert (tennessee_dir / "test_tennessee_simple.py").exists(), "Tennessee simple test missing"
    ...
    if arkansas_dir.exists():
        ...
        assert (arkansas_dir / "test_arkansas_simple.py").exists(), "Arkansas simple test missing"

def test_python_version():
    """Test that Python version is supported."""
    assert sys.version_info >= (3, 8), f"Python {sys.version_info} is not supported. Minimum: 3.8"
