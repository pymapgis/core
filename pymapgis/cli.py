...
try:
    ...
    from pymapgis.cache import stats as stats_api, clear as clear_cache_api, purge as purge_cache_api
    from pymapgis.plugins import (
        load_driver_plugins, load_algorithm_plugins, load_viz_backend_plugins,
        PYMAPGIS_DRIVERS_GROUP, PYMAPGIS_ALGORITHMS_GROUP, PYMAPGIS_VIZ_BACKENDS_GROUP
    )
...
def doctor_command():
    ...
    """
    typer.echo(typer.style("PyMapGIS Doctor: Checking your environment...", fg=typer.colors.CYAN, bold=True))
    issues_found = 0
    ok_color = typer.colors.GREEN
    warning_color = typer.colors.YELLOW
    error_color = typer.colors.RED

    def print_check(label: str, value: str, status: str = "INFO", status_color: str = typer.colors.WHITE):
    ...
    print_check("PyMapGIS Version", pymapgis.__version__, "INFO", ok_color if pymapgis.__version__ != "unknown" else warning_color)
    ...
    for dep in deps_to_check:
        ...
        status, color = ("OK", ok_color) if version != "Not installed" else ("NOT FOUND", error_color)
    ...
    status_proj_lib, color_proj_lib = ("SET", ok_color) if proj_lib else ("NOT SET", warning_color)
    ...
    status_gdal_data, color_gdal_data = ("SET", ok_color) if gdal_data else ("NOT SET", warning_color)
...
def cache_info_command():
    ...
    """
    typer.echo(typer.style("PyMapGIS Cache Information", fg=typer.colors.BRIGHT_BLUE, bold=True))
    try:
        cache_stats = stats_api()
        if not cache_stats:
            typer.echo("Could not retrieve cache statistics. Cache might be disabled or not initialized.")
            return

        for key, value in cache_stats.items():
            friendly_key = key.replace('_', ' ').title()
            if isinstance(value, bool):
                status = typer.style("Enabled", fg=typer.colors.GREEN) if value else typer.style("Disabled", fg=typer.colors.RED)
        ...
        typer.secho(f"Error retrieving cache statistics: {e}", fg=typer.colors.RED, err=True)
...
def cache_clear_command():
    ...
    try:
        clear_cache_api()
        typer.secho("All PyMapGIS caches have been cleared successfully.", fg=typer.colors.GREEN)
    except Exception as e:
...
def cache_purge_command():
    ...
    try:
        purge_cache_api()
        typer.secho("Expired entries purged from requests-cache successfully.", fg=typer.colors.GREEN)
    except Exception as e:
        typer.secho(f"Error purging cache: {e}", fg=typer.colors.RED, err=True)


# --- Plugin Subcommand ---
plugin_app = typer.Typer(name="plugin", help="Manage PyMapGIS plugins.", no_args_is_help=True)
app.add_typer(plugin_app)

def _list_plugins_by_group(group_name: str, loader_func: callable, verbose: bool = False):
    ...
    try:
        ...
        typer.secho(f"  Error loading plugins for group {group_name}: {e}", fg=typer.colors.RED, err=True)
...
def plugin_list_command(
    ...
    """
    typer.echo(typer.style("Discovering PyMapGIS Plugins...", fg=typer.colors.CYAN, bold=True))
    ...
    _list_plugins_by_group(f"Algorithms ({PYMAPGIS_ALGORITHMS_GROUP})", load_algorithm_plugins, verbose)
    _list_plugins_by_group(f"Visualization Backends ({PYMAPGIS_VIZ_BACKENDS_GROUP})", load_viz_backend_plugins, verbose)
